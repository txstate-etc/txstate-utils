 ...
 * @param count     How many of that word exist in this instance
 * @param inclusive Whether to prefix the return string with the count (e.g. '3 ducks' vs 'ducks')
 */
...
function addPluralRule (rule: string | RegExp, replacement: string) {
  pluralRules.push([sanitizeRule(rule), replacement])
}

function addSingularRule (rule: string | RegExp, replacement: string) {
  singularRules.push([sanitizeRule(rule), replacement])
}

function addUncountableRule (word: string | RegExp) {
 ...
 */
function addIrregularRule (single: string, plural: string) {
...
;([
  ...
  [/m[ae]n$/i, 'men'],
  ['thou', 'you']
] as [RegExp, string][]).forEach(function (rule) {
