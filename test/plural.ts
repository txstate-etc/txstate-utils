...
describe('basic plurals tests', function () {
  it('plural', function () {
    BASIC_TESTS.concat(PLURAL_TESTS).forEach(function (test) {
      expect(plural(test[0])).to.equal(test[1])
    })
  })

  it('isPlural', function () {
    BASIC_TESTS.concat(PLURAL_TESTS).forEach(function (test) {
      expect(isPlural(test[1])).to.equal(true)
    })
  })

  it('singular', function () {
    BASIC_TESTS.concat(SINGULAR_TESTS).forEach(function (test) {
      expect(singular(test[1])).to.equal(test[0])
    })
  })

  it('isSingular', function () {
    BASIC_TESTS.concat(SINGULAR_TESTS).forEach(function (test) {
      expect(isSingular(test[0])).to.equal(true)
    })
  })

  it('pluralize -> plural', () => {
    BASIC_TESTS.concat(PLURAL_TESTS).forEach(function (test) {
      expect(pluralize(test[1], 5)).to.equal(test[1]) // stays plural
      expect(pluralize(test[0], 5)).to.equal(test[1]) // becomes plural
    })
  })
  it('pluralize -> singular', () => {
    BASIC_TESTS.concat(SINGULAR_TESTS).forEach(function (test) {
      expect(pluralize(test[0], 1)).to.equal(test[0]) // stays singular
      expect(pluralize(test[1], 1)).to.equal(test[0]) // becomes singular
    })
  })
  it('pluralize prepends count', function () {
    expect(pluralize('test', 5, true)).to.equal('5 tests')
  })

  it('pluralize prepends singular count', function () {
    expect(pluralize('test', 1, true)).to.equal('1 test')
    expect(pluralize('tests', 1, true)).to.equal('1 test')
  })
})
